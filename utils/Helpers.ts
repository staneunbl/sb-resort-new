...
export function emailStringConfirmBooking(config: Config, details: {reservationId: string, bookingDate: Date, checkIn: Date, checkOut: Date, roomType: string, guestName: {firstName: string, lastName: string}, roomBill: number, promoCode?: string, promoCodeValue?: string, discountCode?: string, discountValue?: number, discountType?: string }) {
  
  const discountValue = () => {
    if(details.discountCode && details.discountValue) {
      if (details.discountType === 'percentage') {
        const bill = details.roomBill + (details.roomBill * 0.12)
        return bill * (details.discountValue/ 100)
      }
      return details.discountValue
    }
    return 0
  }

  const promoValueHtml = (details.promoCode && details.promoCodeValue) ? `<tr><td align=left style=width:70%><p style=margin:0;font-size:14px;color:#333>Promo Code (${details.promoCode})<td align=right style=width:30%><p style=margin:0;font-size:14px;color:#22c55e;font-weight:700>- ${details.promoCodeValue}` : ""

  const discountValueHtml = (details.discountCode && details.discountValue) ? `<tr><td align=left style=width:70%><p style=margin:0;font-size:14px;color:#333>Discount <i>${details.discountCode}</i>${details.discountType === 'percentage' ? ` (${details.discountValue}% off)` : ""}<td align=right style=width:30%><p style=margin:0;font-size:14px;color:#22c55e;font-weight:700>- ₱${formatCurrencyJP(discountValue())}` : ""

  const totalHtml = (details.discountCode && details.discountValue) ? `<tr><td align=left style="vertical-align:top;"><p style=margin:0;font-size:14px;color:#fff>TOTAL<td align=right><h2 style=margin:0;color:#fff;font-weight:700>₱${formatCurrencyJP((details.roomBill + (details.roomBill * 0.12) - discountValue()))}</h2><h4 style=margin:0;color:#ADD3EC;font-weight:700><s>₱${formatCurrencyJP((details.roomBill + (details.roomBill * 0.12)))}</s></h4>` : `<tr><td align=left><p style=margin:0;font-size:14px;color:#fff>TOTAL<td align=right><h2 style=margin:0;color:#fff;font-weight:700>₱${formatCurrencyJP((details.roomBill + (details.roomBill * 0.12) - discountValue()))}`


  return `
    <!doctypehtml><meta charset=UTF-8><meta content="width=device-width,initial-scale=1"name=viewport><meta content="IE=edge"http-equiv=X-UA-Compatible><meta content="text/html charset=UTF-8"http-equiv=Content-Type><title>${config.CompanyName}</title><style>@import url(https://fonts.googleapis.com/css2?family=Montserrat:ital,wght@0,100..900;1,100..900&display=swap);@media only screen and (max-width:600px){.container{width:100%!important;padding:0!important}.content{padding:10px!important}.two-column{width:100%!important}.two-column td{display:block!important;width:100%!important;text-align:center;padding-bottom:12px}.responsive-logo{width:100%;max-width:100%;height:auto}}</style><body style=margin:0;padding:0;font-family:Montserrat,Arial,sans-serif;background-color:#d9d9d9><table style=background-color:#d9d9d9;padding:24px width=100% cellpadding=0 cellspacing=0><tr><td align=center><table style=background-color:#d9d9d9;padding:24px;border-radius:10px width=600 cellpadding=0 cellspacing=0 class=container><tr><td><table style=background-color:#fff;border-radius:10px;width:100% cellpadding=0 cellspacing=0><tr><td align=center style="padding:20px 0;padding-top:32px"style=border-radius:10px><img src=${config.CompanyLogo} style=display:block alt="Company Logo"width=24><p style=margin-bottom:0;color:#838383>${config.CompanyName}<tr style=background-color:#fff><td style=padding:48px;color:#333;text-align:center class=content><img src="https://uaukaqfnisyvjiondsrq.supabase.co/storage/v1/object/public/images/email/email_booking_success-min.png" style=width:360px;height:auto class=responsive-logo><h1 style="font-size:24px;margin:0 0 20px;margin-top:20px;color:#5aa6d8">Your reservation has been confirmed.</h1><p style="font-size:16px;line-height:1.5;margin:0 0 20px;">Hello <span style="font-weight:600">${details.guestName.firstName}</span>! This is a confirmation email for your reservation at ${config.CompanyName}. Your reservation details and summary are provided below for your convenience.<p style="font-size:16px;line-height:1.5;margin:0 0 20px">Thank you for choosing ${config.CompanyName}. We look forward to hosting you.<table style=background-color:#d5e8f5;padding:12px;border-radius:10px width=100% cellpadding=0 class=two-column><tr><td align=left style=width:50%;padding-bottom:12px><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Reservation Number<p style=margin:0;margin-top:4px>${details.reservationId}<td align=left style=width:50%;padding-bottom:12px><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Booking Date<p style=margin:0;margin-top:4px>${format(details.bookingDate, "MMM dd, yyyy")}<tr><td align=left style=width:50%;padding-bottom:12px><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Check-In Date<p style=margin:0;margin-top:4px>${format(details.checkIn, "MMM dd, yyyy")}<td align=left style=width:50%;padding-bottom:12px><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Check-Out Date<p style=margin:0;margin-top:4px>${format(details.checkOut, "MMM dd, yyyy")}<tr><td align=left style=width:50%><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Room Type<p style=margin:0;margin-top:4px>${details.roomType}<td align=left style=width:50%><p style="font-weight:600;margin:0 auto;color:#5aa6d8">Guest Name<p style=margin:0;margin-top:4px>${details.guestName.firstName} ${details.guestName.lastName}</table><hr style="border-color:#d9d9d9;border-width:1px;margin:16px auto"><table style=padding:12px width=100%><tr><th align=left style=width:70%><p style="margin:4px auto;font-weight:600;color:#5aa6d8">Description<th align=right><p style="margin:4px auto;font-weight:600;color:#5aa6d8">Price<tr><td align=left style=width:70%><p style=margin:0;font-size:14px;color:#333>${details.roomType} Room<td align=right style=width:30%><p style=margin:0;font-size:14px;color:#333;font-weight:700>₱${formatCurrencyJP(details.roomBill)}<tr><td align=left style=width:70%><p style=margin:0;font-size:14px;color:#333>VAT (12%)<td align=right style=width:30%><p style=margin:0;font-size:14px;color:#333;font-weight:700>₱${formatCurrencyJP(details.roomBill * 0.12)}${promoValueHtml}${discountValueHtml}</table><hr style="border-color:#d9d9d9;border-width:1px;margin:16px auto;width:90%"><table style=border-radius:10px;padding:12px;background-color:#5aa6d8 width=100%>${totalHtml}</table><table style=margin:0;margin-top:20px width=100% cellpadding=0><tr><td><h3 style=color:#5aa6d8;margin-bottom:0>Any questions?</h3><tr><td><p>Contact us at <span style=color:#5aa6d8;font-weight:700>${"0912345678"}</span> or send us an email at <span style=color:#5aa6d8;font-weight:700>${config.CompanyEmail}</span> for inquiries and assistance.</table><tr style=background-color:#e4e4e4><td align=center style=padding:20px;color:#999;font-size:12px><p style=margin:0;font-weight:700>${config.CompanyName}<p style=margin:0>${config.CompanyAddress}<p style=margin:0>${config.CompanyEmail}<p style=margin:0>${config.CompanyContact}</table></table></table>
  `
}
